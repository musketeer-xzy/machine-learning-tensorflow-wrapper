...
def compute_logits(inputs, batch_size=-1):
    ...
    logits.append(tf.reshape(logits[-1], shape=[batch_size, temp_shape[1], temp_shape[2] * temp_shape[3]]))
    ...
    logits.append(FullyConnected.build_model(logits[-1], config.encoder('fullyconnected')))
    ...
    with tf.Session() as sess:
        ...
        for x in range(1, train_config['max_epoch'] + 1, 1):
            ...
            if batch_id == 0 and len(current_loss) > 0:
                ...
                if len(mean_loss_list) > 5 and mean_loss_list[-1] > np.mean(mean_loss_list[-5:]):
