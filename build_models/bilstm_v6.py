...
def compute_logits(inputs, batch_size=-1):
    ...
    # print(temp_shape)tr
    logits.append(tf.reshape(logits[-1], shape=[batch_size, temp_shape[1], temp_shape[2]]))
    # print(logits[-1].get_shape().as_list())
    # logits.append(FullyConnected.build_model(logits[-1], config.encoder('fullyconnected_1')))
    # print(logits[-1].get_shape().as_list())
    # logits.append(tf.reshape(logits[-1], shape=[batch_size, -1, config.encoder('fullyconnected_1')['num_outputs']]))
    # print(logits[-1].get_shape().as_list())
    ...
    # logits.append(FullyConnected.build_model(logits[-1], 'fullyconnected'))
    # logits.append(tf.reshape(logits[-1], shape=[config.encoder('fullyconnected_1')['num_outputs'], 150, None]))
    logits.append(BiDirectionLSTM.build_model(logits[-1], config.encoder('bidirection_lstm')))
    ...
    with tf.Session() as sess:
        ...
        for x in range(1, train_config['max_epoch'] + 1, 1):
            ...
            if batch_id == 0 and len(current_loss) > 0:
                ...
                loss_max = np.max(current_loss)
                loss_min = np.min(current_loss)
                t = threading.Thread(target=draw_loss, args=(
                    datetime.now(), loss_mean, loss_max, loss_min, learning_rate, mode))
                t.start()
                if mode == 'w':
                    mode = 'a'
...
def draw_loss(time, mean_loss, max_loss, min_loss, learning_rate, mode):
    with open('loss.txt', mode) as _file:
        _file.write('%s,%s,%s,%s,%s%s' % (time, mean_loss, max_loss, min_loss, learning_rate, os.linesep))
