...
config = Config('bidirection_lstm.conf')
...
def compute_logits(inputs, batch_size=-1):
    ...
    logits = BiDirectionLSTM.build_model(inputs, config.encoder('bidirection_lstm'))
    logits = tf.reshape(logits, shape=[-1, logits.get_shape()[-1]])
    logits = FullyConnected.build_model(logits, config.encoder('fullyconnected'))
...
def build_model(batch_size=None):
    ...
    return loss, accuracy, global_step, learning_rate, learning_rate_decay, learning_rate_init, optimizer, inputs, \
           outputs, seq_len
...
def train(get_next_batch, expand_batch):
    ...
    _outputs, _seq_len = build_model(batch_size=500)
...
def client_predict(batch_size, func_inputs):
    _predict, _inputs, _seq_len = build_predict_model(batch_size=batch_size)
    ...
    with tf.Session() as sess:
        ...
        if checkpoint and checkpoint.model_checkpoint_path:
            ...
            sess.run(tf.global_variables_initializer())
        inputs, tag = func_inputs()
        predict_value = sess.run([_predict],
                                 feed_dict={_inputs: inputs, _seq_len: np.ones(len(inputs)) * 150})
        return predict_value[0], tag
